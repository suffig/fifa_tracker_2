...
function handleVisibilityChange() {
    ...
    if (!isAppVisible && wasVisible) {
        ...
        // Session prüfen und je nach Status nur App oder Login neu rendern!
        supabase.auth.getSession().then(({data: {session}}) => {
            if(session) {
                // Nur die App neu initialisieren (nicht das Loginfenster!)
...
function setupLogoutButton() {
    ...
    if (logoutBtn) {
        logoutBtn.onclick = async () => {
            await signOut();
            // Statt Reload: State, UI und Subscriptions zurücksetzen
            liveSyncInitialized = false;
            tabButtonsInitialized = false;
            cleanupRealtimeSubscriptions();
            if (inactivityCleanupTimer) {
            ...
            // Optional: Reset aller Datenmodule
